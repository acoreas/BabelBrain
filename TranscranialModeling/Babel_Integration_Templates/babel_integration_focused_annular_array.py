...
from TranscranialModeling.tx_geometries import generate_annular_array_tx
...
class BabelFTD_Simulations(BabelFTD_Simulations_BASE):
    ...
    def __init__(
        ...
        **kargs,
        ...
        if FocalLength <= 0.0:
            raise ValueError(
                "FocalLength must be a positive value, got %f" % (FocalLength)
            )
        if len(InDiameters) == 0:
            raise ValueError(
                "InDiameters must be a list/array of positive values, got %s"
                % (InDiameters)
            )
        if len(OutDiameters) == 0:
            raise ValueError(
                "OutDiameters must be a list/array of positive values, got %s"
                % (OutDiameters)
            )
        self._ZSteering = ZSteering
        ...
        print(
            "BabelIntegrationANNULAR_ARRAY parameters (Aperture,FocalLength,InDiameters,OutDiameters):",
            Aperture,
            FocalLength,
            InDiameters,
            OutDiameters,
    ...
    def GenerateSTLTx(self, prefix):
        ...
        for VertDisplay, FaceDisplay in zip(
            ...
            TxVert = TxVert.T[:, :3]
            TxElemCenters.append(
                np.mean(TxVert, axis=0)
            )  # In ring Tx, we use the center of mass of all vertices
    ...
    def AddSaveDataSim(self, DataForSim):
        ...
        DataForSim["BasePhasedArrayProgramming"] = (
            self._SIM_SETTINGS.BasePhasedArrayProgramming
        )
...
class SimulationConditions(SimulationConditionsBASE):
    ...
    def GenTx(self, bOrigDimensions=False, PPWSurface=8):
        ...
        TxRC = generate_annular_array_tx(
            self._Frequency,
            self._FocalLength / fScaling,
            self._InDiameters / fScaling,
            self._OutDiameters / fScaling,
    ...
    def CalculateRayleighFieldsForward(self, deviceName="6800"):
        ...
        center[0, 0] = (
            self._XDim[self._FocalSpotLocation[0]] + self._TxMechanicalAdjustmentX
        )
        center[0, 1] = (
            self._YDim[self._FocalSpotLocation[1]] + self._TxMechanicalAdjustmentY
        )
        center[0, 2] = (
            self._ZDim[self._FocalSpotLocation[2]]
            + self._ZSteering
            + self._TxMechanicalAdjustmentZ
        )
        print("center", center)
        ...
        if self._bDisplay:
            ...
            plt.subplot(1, 2, 1)
            plt.imshow(
                np.abs(self._SourceMapRayleigh) / 1e6,
                vmin=np.abs(self._SourceMapRayleigh[RegionMap]).min() / 1e6,
                cmap=plt.cm.jet,
            )
            plt.colorbar()
            ...
            plt.imshow(
                (
                    np.abs(u2[:, self._FocalSpotLocation[1], :]).T
                    + self._MaterialMap[self._FocalSpotLocation[0], :, :].T
                    * np.abs(u2[self._FocalSpotLocation[0], :, :]).max()
                    / 10
                )
                / 1e6,
                extent=[
